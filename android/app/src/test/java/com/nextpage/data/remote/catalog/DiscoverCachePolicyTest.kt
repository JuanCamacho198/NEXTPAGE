...
class DiscoverCachePolicyTest {
    ...
    private fun entity(key: String, payload: String, fetchedAt: Long, ttl: Long) =
        DiscoverCacheEntity(key, payload, fetchedAt, ttl)

    // ── CL1: prune deletes TTL-dead rows only ────────────────────────────

    @Test
    fun pruneBoundary_exactNow_keptJustExpired_deleted() = runTest {
    ...
    @Test
    fun prune_keepsUnexpiredRows() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun purge_deletesV1V2Keys_keepsV3() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun read_returnsStaleResidentWithoutEvicting() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun swr_staleServe_startsBackgroundRefresh() = runTest {
    ...
    @Test
    fun atCap_evictsExpiredRowAndAdmitsWrite() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun unexpiredRow_notEvicted_writeSkipped() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun singleRowOverCap_isNeverStored() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun replace_isMeasuredNetOfExistingKey() = runTest {
        val dao = FakeDiscoverCacheDao()
    ...
    @Test
    fun eviction_candidatesOnlyDiscoverCache() = runTest {
        val cacheOnlyMethods = setOf(
            "getByKey", "put", "deleteByKey", "count",
            "payloadBytes", "payloadBytesForKey", "deleteExpired",
            "deleteLegacyNamespaces", "deleteAll"
        )
        val declared = DiscoverCacheDao::class.java.declaredMethods.map { it.name }.toSet()
        assertTrue(
