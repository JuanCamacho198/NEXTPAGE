...
use crate::models::CollectionDto;
    ...
    pub fn create_collection(repo: &LibraryRepository, name: &str, color: Option<&str>) -> AppResult<CollectionDto> {
        let name = name.trim();
        if name.is_empty() {
    ...
    pub fn add_book_to_collection(repo: &LibraryRepository, book_id: &str, collection_id: i64) -> AppResult<()> {
    ...
    pub fn remove_book_from_collection(repo: &LibraryRepository, book_id: &str, collection_id: i64) -> AppResult<()> {
        let book_id = book_id.trim();
        if book_id.is_empty() {
            return Err(AppError::MissingBookId);
        }
    ...
    pub fn get_book_collections(repo: &LibraryRepository, book_id: &str) -> AppResult<Vec<CollectionDto>> {
        let book_id = book_id.trim();
        if book_id.is_empty() {
            return Err(AppError::MissingBookId);
        }
