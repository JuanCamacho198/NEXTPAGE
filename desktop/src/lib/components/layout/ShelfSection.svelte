...
<div class="space-y-3">
  <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
    <div class="flex flex-wrap items-center gap-2" data-testid="shelf-tabs">
      {#each appState.SHELF_TAB_OPTIONS as tabOption}
        <button
          ...
          class={`rounded-md border px-2.5 py-1 text-xs font-medium transition-colors ${appState.shelfQueryState.tab === tabOption.key ? "border-[var(--color-primary)] bg-[color:color-mix(in_srgb,var(--color-primary)_12%,var(--color-surface))] text-[var(--color-primary)]" : "border-[color:var(--color-border)] bg-[var(--color-background)] text-[var(--color-text-muted)] hover:bg-[color:var(--color-border)]"}`}
          onclick={() => {
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="inline-flex rounded-md border border-[color:var(--color-border)] bg-[var(--color-background)] p-1" data-testid="shelf-view-toggle">
        <button
          type="button"
          class={`flex h-7 w-8 items-center justify-center rounded px-0 py-1 text-xs font-medium ${appState.shelfQueryState.viewMode === "grid" ? "bg-[var(--color-surface)] text-[var(--color-primary)]" : "text-[var(--color-text-muted)]"}`}
          onclick={() => {
        ...
        <button
          type="button"
          class={`flex h-7 w-8 items-center justify-center rounded px-0 py-1 text-xs font-medium ${appState.shelfQueryState.viewMode === "list" ? "bg-[var(--color-surface)] text-[var(--color-primary)]" : "text-[var(--color-text-muted)]"}`}
          onclick={() => {
  ...
  {#if appState.shelfSortToken}
    <p class="text-xs text-[var(--color-text-muted)]">{appState.t("home.shelfSortFromQuery", { value: appState.shelfSortToken })}</p>
  {/if}
  ...
  <p class="text-xs text-[var(--color-text-muted)]">{appState.t("home.shelfResults", { count: appState.shelfBooks.length, total: appState.myShelfBooks.length })}</p>
</div>

{#if appState.myShelfBooks.length === 0}
  <p class="text-sm text-[var(--color-text-muted)]">{appState.t("home.myShelfPlaceholder")}</p>
{:else if appState.shelfBooks.length === 0}
  <p class="text-sm text-[var(--color-text-muted)]">{appState.t("home.shelfNoResults")}</p>
{:else}
  ...
  <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={shelfDetail.title}>
    <div class="w-full max-w-xl rounded-xl border border-[color:var(--color-border)] bg-[var(--color-surface)] p-4 shadow-xl">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <h3 class="line-clamp-2 text-lg font-semibold text-[var(--color-primary)]">{shelfDetail.title}</h3>
          <p class="truncate text-sm text-[var(--color-text-muted)]">{shelfDetail.author || appState.t("app.unknownAuthor")}</p>
        </div>
