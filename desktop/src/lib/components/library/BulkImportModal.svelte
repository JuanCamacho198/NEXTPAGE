<script lang="ts">
  ...
  const getStatusKey = (status: string): MessageKey => {
...
{#if open}
  ...
  >
    <div class="flex max-h-[90vh] w-full max-w-3xl flex-col rounded-xl border border-[color:var(--color-border)] bg-[var(--color-surface)] p-5 shadow-lg">
      <div class="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 id="bulk-import-title" class="text-lg font-semibold text-[var(--color-primary)]">
            {t("library.bulkImport.title")}
          </h2>
          <p class="text-sm text-[var(--color-text-muted)]">{t("library.bulkImport.description")}</p>
        </div>
