...
<div class="flex gap-3 items-start rounded-xl border border-(--color-border) bg-(--color-surface,#fff) p-3">
  <div class="w-14 h-14 shrink-0">
    {#if profile.avatarUrl && !avatarBroken}
      ...
      />
    {:else}
      <div class="w-full h-full rounded-full flex items-center justify-center text-sm font-bold text-(--color-primary) bg-(--color-primary)/12 border border-(--color-border)" aria-hidden="true">
  ...
  <div class="min-w-0 flex-1">
    ...
    <p class="my-0.5 mb-2 text-sm text-(--color-primary) wrap-break-word">{isProfileLoading ? t("settings.profile.loading") : profile.name}</p>

    <p class="m-0 text-[11px] text-(--color-text-muted,#6b7280)">{t("settings.profile.emailLabel")}</p>
    <p class="my-0.5 mb-2 text-sm text-(--color-primary) wrap-break-word">{isProfileLoading ? t("settings.profile.loading") : profile.email}</p>
