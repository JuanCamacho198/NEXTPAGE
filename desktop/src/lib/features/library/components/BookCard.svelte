    ...
    <button type="button" class="min-w-0 flex-1 text-left" onclick={onSelect}>
      <div class="flex items-start gap-3">
        ...
        >
          {#snippet fallback()}
            <div
              class={`${compact ? 'h-14 w-10' : 'h-16 w-12'} flex items-center justify-center rounded bg-(--color-surface) text-[10px] uppercase tracking-widest text-(--color-text-muted)`}
            >
              {t('library.cover')}
            </div>
          {/snippet}
        ...
        <div class="min-w-0 flex-1 space-y-1">
          ...
          {#if showProgress}
            <div class="mt-2">
              <div
                class="mb-1 flex items-center justify-between text-[11px] text-(--color-text-muted)"
              >
