  ...
  <div class="absolute inset-0 z-10 flex items-center justify-center bg-(--color-background)">
    {#if loadProgressMax > 0 && loadProgress < loadProgressMax}
      <div class="flex flex-col items-center gap-2 min-w-60">
        <span class="text-sm text-(--pdf-reader-text,#64748b)">{t("pdf.loading")}</span>
        <div class="w-full h-1.5 bg-(--color-border,#1E293B) rounded-full overflow-hidden">
          <div
        ...
        <span class="text-xs text-(--pdf-reader-text,#94A3B8) opacity-80">{Math.round((loadProgress / loadProgressMax) * 100)}%</span>
      </div>
    {:else}
      {t("pdf.loading")}
    {/if}
