...
<aside class="w-60 bg-[var(--pdf-reader-surface-bg,var(--color-surface))] border-r border-(--color-border) overflow-y-auto shrink-0 max-sm:w-[min(240px,70vw)]">
  <h3 class="m-0 px-3 py-3 text-sm font-semibold border-b border-(--color-border) text-[var(--pdf-reader-text,var(--color-primary))]">
      ...
      {#each flatOutline as entry (entry.item.id)}
        <li>
          <button
            ...
            class="w-full px-3 py-2.5 border-none bg-transparent text-left cursor-pointer text-xs leading-[1.4] wrap-break-word text-[var(--pdf-reader-text,var(--color-primary))] disabled:opacity-55 disabled:cursor-default hover:not-disabled:bg-[color-mix(in_srgb,var(--color-primary)_8%,var(--color-surface))]"
          >
            {entry.item.title}
          </button>
        </li>
