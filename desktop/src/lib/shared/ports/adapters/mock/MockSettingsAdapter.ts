...
export class MockSettingsAdapter implements SettingsPort {
  ...
  async getAppSettings(): Promise<AppSettingDto[]> { return [...this.#app.values()]; }
  async upsertAppSettings(s: AppSettingDto[]): Promise<void> { for (const e of s) this.#app.set(e.key, e); }
