...
export function sha256Hex(bytes: Uint8Array): Promise<string> {
  ...
  return crypto.subtle
    .digest('SHA-256', source.buffer as ArrayBuffer)
    .then((d) => [...new Uint8Array(d)].map((b) => b.toString(16).padStart(2, '0')).join(''));
}
/** Recoverable candidates; local absence never emits deletion. */
export function classifyRecoveryCatalog(
  remote: SupabaseUserBookRow[],
  localIds: ReadonlySet<string>,
): { downloadable: SupabaseUserBookRow[]; deleted: SupabaseUserBookRow[] } {
  const deleted = remote.filter((r) => r.lifecycle === 'deleted');
  const downloadable = remote.filter(
    (r) => !localIds.has(r.id) && r.lifecycle !== 'deleted' && r.lifecycle !== 'unavailable',
  );
...
export function toSyncError(err: unknown, bookId?: string): SyncError {
  ...
  const code = /401|unauthorized|token|sign in|authentication required/i.test(lower)
    ? 'AUTH_REQUIRED'
    : /403|permission/i.test(lower)
      ? 'PERMISSION_DENIED'
...
export async function importRecoveredBook(
  row: SupabaseUserBookRow,
  deps: ImportDeps,
  ...
  try {
    await deps.persist(row.id, bytes, {
      title: row.title,
      author: row.author ?? '',
      format: row.format,
    });
...
export async function importSelectedBooks(
  rows: SupabaseUserBookRow[],
  deps: ImportDeps,
): Promise<ImportResult[]> {
