...
function parseResources(value: unknown): string[] {
  if (!Array.isArray(value) || value.length === 0) {
    throw addonFetchError(AddonFetchErrorCode.INVALID_MANIFEST, "resources must be a non-empty array");
  }
...
function parseManifestObject(value: unknown): AddonManifest {
  ...
  if (!Array.isArray(value.catalogs) || value.catalogs.length === 0) {
    throw addonFetchError(AddonFetchErrorCode.INVALID_MANIFEST, "catalogs must be a non-empty array");
  }
...
export function validateManifest(bytes: Uint8Array, contentType: string | null): AddonManifest {
  if (bytes.byteLength > MAX_MANIFEST_BYTES) {
    throw addonFetchError(AddonFetchErrorCode.TOO_LARGE, `manifest exceeds ${MAX_MANIFEST_BYTES} bytes`);
  }
  if (!isJsonContentType(contentType)) {
    throw addonFetchError(AddonFetchErrorCode.BAD_CONTENT_TYPE, `content type is not JSON: ${contentType}`);
  }
  let parsed: unknown;
  try {
