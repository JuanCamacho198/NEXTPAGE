...
export function isValidReadingStateEnvelope(value: unknown): value is ReadingStateEnvelope {
  ...
  return (
    typeof envelope.bookId === 'string' &&
    ['to_read', 'reading', 'completed'].includes(envelope.state ?? '') &&
    typeof envelope.percentage === 'number' &&
    envelope.percentage >= 0 &&
    envelope.percentage <= 100 &&
    typeof stateVersion === 'number' &&
    Number.isInteger(stateVersion) &&
    stateVersion >= 0 &&
    typeof envelope.eventUpdatedAt === 'string' &&
    Number.isFinite(Date.parse(envelope.eventUpdatedAt)) &&
    typeof envelope.deviceId === 'string' &&
    envelope.deviceId.length > 0
  );
}

export function shouldApplyReadingState(
  local: ReadingStateEnvelope | null,
  remote: unknown,
): remote is ReadingStateEnvelope {
  return (
    isValidReadingStateEnvelope(remote) &&
    (local === null || compareReadingState(remote, local) > 0)
  );
}
