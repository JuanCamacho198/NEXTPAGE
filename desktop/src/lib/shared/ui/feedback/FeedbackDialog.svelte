...
<script lang="ts">
  ...
  /**
   * Escape and an outside pointerdown now dismiss the dialog, and they carry
   * the SAME bookkeeping as the X button: mark the event dismissed so the
   * dialog never re-nags, then hand the close to the parent. Before the
   * bits-ui swap neither interaction did anything; the swap makes the
   * standard dialog contract reachable, so it is closed deliberately here
   * rather than left as a library default that skips the dismiss semantics.
   * `onOpenChange` is intentionally NOT bound: the two paths above and the
   * explicit buttons are the only closers, so `onDismiss` fires once.
   */
  function handleDismissInteraction(_event: Event): void {
    if (eventId) markDismissed(eventId);
    onDismiss(eventId);
    open = false;
  }
    ...
    <Dialog.Portal>
      <Dialog.Overlay class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <Dialog.Content
          ...
          onEscapeKeydown={handleDismissInteraction}
          onInteractOutside={handleDismissInteraction}
        >
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <Dialog.Title id="feedback-title" level={2}>
                  {#snippet child({ props })}
                    <h2 {...props} class="text-lg font-bold">{FEEDBACK_TITLE}</h2>
                  {/snippet}
                </Dialog.Title>
              </div>
            </div>
