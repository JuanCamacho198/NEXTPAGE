...
const { mockDestroy, mockGetOutline, mockPdfDocument, mockGetFileBytes } = vi.hoisted(() => {
...
describe('pdfStreaming — Document Cache', () => {
  ...
  it('clearDocumentCache destroys all cached documents and clears', () => {
    setCachedDocument('/a.pdf', {
      document: makeMockDocument(),
      outline: [],
      outlineLoaded: false,
    });
    setCachedDocument('/b.pdf', {
      document: makeMockDocument(),
      outline: [],
      outlineLoaded: false,
    });
    setCachedDocument('/c.pdf', {
      document: makeMockDocument(),
      outline: [],
      outlineLoaded: false,
    });
...
describe('pdfStreaming — LRU Eviction', () => {
  ...
  it('re-setting same key does not trigger eviction', () => {
    for (let i = 0; i < 8; i++) {
      setCachedDocument(`/book-${i}.pdf`, {
        document: makeMockDocument(),
        outline: [],
        outlineLoaded: false,
      });
    }
    setCachedDocument('/book-0.pdf', {
      document: makeMockDocument(),
      outline: [],
      outlineLoaded: true,
    });
  ...
  it('calls document.getOutline() and caches the result', async () => {
    const rawOutline = [
      { title: 'Chapter 1', dest: 'page=1', items: [] },
      {
        title: 'Chapter 2',
        dest: 'page=5',
        items: [{ title: 'Section 2.1', dest: 'page=5', items: [] }],
      },
    ];
