...
const { mockTextLayer } = vi.hoisted(() => {
  ...
  const ml = vi.fn().mockImplementation(function ({
    container,
    viewport,
  }: {
...
import {
  SafeTextLayer,
  type SafeTextLayerParams,
} from '$lib/features/reader/viewer-pdf/safeTextLayer';

// ── Helpers ───────────────────────────────────────────────
function createViewport(
  overrides: Partial<{ scale: number; width: number; height: number }> = {},
...
describe('SafeTextLayer', () => {
  ...
  describe('constructor', () => {
    ...
    it('sets container width and height from viewport dimensions', () => {
      new SafeTextLayer({
        container,
        viewport: createViewport({ width: 800, height: 1100 }),
        textContentSource: makeEmptyTextSource(),
      });
