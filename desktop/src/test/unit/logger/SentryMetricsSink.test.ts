...
describe('SentryMetricsSink', () => {
  ...
  it('maps duration events to a distribution with tags', () => {
    ...
    expect(opts.tags).toMatchObject({ platform: 'desktop', source: 'reader' });
  });

  it('maps count-only events to increment', () => {
    const sink = new SentryMetricsSink(FULL_SETTINGS);
    sink.handle(makeEvent({ name: 'import_start', bucketedDurationMs: undefined }));
    expect(distribution).not.toHaveBeenCalled();
    expect(increment).toHaveBeenCalledTimes(1);
    expect(increment.mock.calls[0][0]).toBe('import_start');
  });

  it('adds an _error increment with truncated error_code on failure', () => {
    const sink = new SentryMetricsSink(FULL_SETTINGS);
    sink.handle(
      makeEvent({
    ...
    expect(increment).toHaveBeenCalledTimes(1);
    const [name, , opts] = increment.mock.calls[0];
    expect(name).toBe('ipc_call_error');
    expect(opts.tags['error_code']).toBe('IMPORT_F');
    expect(opts.tags['error_code']).not.toContain('TOO_LARGE');
  });
