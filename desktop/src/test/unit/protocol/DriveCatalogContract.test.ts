...
import { DRIVE_BOOKS_PATH, DRIVE_SCOPE, canonicalBookName, canonicalBookPath, mergeCatalogVersion, reconcileLegacyReference, coverError, type SyncErrorCode } from '$lib/shared/protocol/DriveCatalogContract';
...
describe('Drive catalog contract', () => {
  ...
  it('ignores stale events and accepts equal/newer versions', () => { const current = { catalogVersion: 4, lifecycle: 'available' as const }; expect(mergeCatalogVersion(current, { catalogVersion: 3, lifecycle: 'deleted' })).toBe(current); expect(mergeCatalogVersion(current, { catalogVersion: 4, lifecycle: 'deleted' }).lifecycle).toBe('deleted'); });
