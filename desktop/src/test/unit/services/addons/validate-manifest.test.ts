...
describe("assertHttpsInstallUrl", () => {
  ...
  it("rejects non-HTTP(S) schemes with HTTPS_REQUIRED", () => {
    for (const url of ["ftp://example.com/m.json", "file:///etc/manifest.json", "javascript:alert(1)"]) {
      try {
      ...
      it(`fixture ${name} yields the shared expected outcome`, () => {
        ...
        try {
          if (fixture.url !== undefined) {
            ...
            validateManifest(new Uint8Array(fixture.rawBytesLength), fixture.contentType ?? "application/json");
          } else {
